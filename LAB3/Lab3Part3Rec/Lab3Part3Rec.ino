...
void setup() {
  ...
  //pinMode (D3, INPUT_PULLUP); // initialize Port D3 as a "PULLED UP" input switch
  //int iNum = digitalPinToInterrupt(D3);
  //attachInterrupt(iNum, &buttonInterrupt, RISING); //attach an interrupt to call buttonInterrupt() whenever pin D3 switches to HIGH
...
void loop() {
  // Create buffers to hold up to 16 entered bytes and 64 1-byte instructions
  if (intUpdate) {
    if (msg) {
      //buffer = "New Message!";
      //inputLength = 12;
