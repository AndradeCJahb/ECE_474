...
// Date:        06/25/24
// Description: This file toggles an off-board LED on and off at 1 second intervals by directly accessing TIMG and GPIO registers
...
void setup() {
  ...
  *((volatile uint32_t*) GPIO_OUT_REG) &= ~(1 << LED_PIN);    // Ensure the LED (D2) is off to start
  ...
  // Configure intermediate value to hold a timer that is to be enabled, have an incrementing counter, and have a divider of 80
  uint32_t config_value = 0;
  config_value |= 1 << 31;  // set enable flag
  config_value |= 1 << 30;  // set increase flag
  config_value |= 80 << 13; // set divider to 80

  // Clear upper 19 bits, then replace the bits (upper 19) with previously built config_value variable
